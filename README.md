...
**完全オフラインで動く、リアルタイム日本語音声入力ツール。** 喋った内容を [faster-whisper](https://github.com/SYSTRAN/faster-whisper) で文字起こしし、[Ollama](https://ollama.com/) 上のローカル LLM がフィラーと句読点だけを整形して、アクティブなアプリにそのまま貼り付ける。

すべての処理が自分の PC の中で完結する。ネット接続なし、API キーなし、月額料金ゼロ。会議メモも独り言も、一切外部サーバーに送信されない。

## なぜ Kotonoha？

- **プライバシー重視**: 音声もテキストも外部に送らない。社外秘の議事録でも安心して使える
- **完全オフライン**: 飛行機・新幹線・電波の悪い場所でもフル機能で動作
- **月額料金ゼロ**: Whisper も Ollama も無料モデル。API 従量課金なし
- **日本語に最適化**: 日本語向け initial prompt + ユーザー辞書で固有名詞も高精度
- **言い回しを壊さない**: 「最小整形」プロンプトで敬語化もカジュアル化もしない。元のニュアンスを維持
- **アプリ別プロンプト**: コードエディタでは敬語なし、LINE では少し丁寧に、など使い分け可能

## 特長

- **常駐オーバーレイ**: 画面下に薄いピル型のバーを表示。録音中は波形が動く
- **ホットキー起動**: Right Alt ダブルタップで表示切替、シングルタップで録音開始／停止
- **リアルタイム文字起こし**: 録音中でも途中経過を逐次表示
- **最小整形 LLM**: フィラー除去と句読点補完だけ。言い回しは一切変えない
- **アプリ別プロンプト切替**: Claude Code / VSCode / Cursor / LINE / Discord などアクティブウィンドウごとに整形ルールを変える
- **ユーザー辞書**: 固有名詞の誤認識を `user_dictionary.json` に登録すると Whisper の `initial_prompt` に反映
- **自動貼り付け**: 文字起こし結果をクリップボード経由で現在の入力欄に Ctrl+V で即投入
- **システムトレイ常駐**: バックグラウンドで動き続ける
...
| Right Alt + Ctrl | 終了 |
| バー上のマイクアイコン | 録音開始／停止 |
| バー本体をドラッグ | バーを好きな位置に移動（位置は次回起動時に復元） |
| バー右上の − | マイク 1 個分にミニ化（グロウも小さく追従） |
| バー右上の ＋ | ミニ化からフルサイズに復帰 |
| バー下部中央の × | バーを隠す（常駐は継続） |
| システムトレイ | 右クリックメニューから操作 |

> ドラッグ・ミニ化・ ＋ ボタンはマウスカーソルがバー周辺に近づいたときだけフェード表示される。
